...
namespace MyChat.Client.ClassExtender
{
    ...
    internal static class UserExtender
    {
        ...
        /// <summary>
        /// Converts a list of <see cref="UserContract"/> to a list of <see cref="User"/>.
        /// </summary>
        /// <param name="users">The list of <see cref="UserContract"/> to convert.</param>
        /// <returns>The list of <see cref="User"/> converted.</returns>
        public static IReadOnlyCollection<User> FromContracts(this IReadOnlyCollection<UserContract> users)
        {
            if (users == null)
            {
            ...
            return users.Where(predicate: user => user != null).Select(selector: user => user.FromContract()).ToArray();
        }
